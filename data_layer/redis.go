...
func (r *RedisLayer) Init(conf *config.CacheConfig, prefix string, ctx context.Context) error {
...
func (r *RedisLayer) initScripts() error {
	...
	result := r.client.ScriptLoad(r.ctx, batchKeyExistScript)
	if result.Err() != nil {
		r.logger.CtxError(r.ctx, "[initScripts] init script 1 error: %v", result.Err())
		return result.Err()
	}
	r.batchExistSha = result.Val()
	r.logger.CtxInfo(r.ctx, "[initScripts] init batch exist script sha1: %s", r.batchExistSha)

	result = r.client.ScriptLoad(r.ctx, cleanCacheScript)
	if result.Err() != nil {
		r.logger.CtxError(r.ctx, "[initScripts] init script 2 error: %v", result.Err())
		return result.Err()
	}
	r.cleanCacheSha = result.Val()
	r.logger.CtxInfo(r.ctx, "[initScripts] init clean cache script sha1: %s", r.cleanCacheSha)
	return nil
}
...
func (r *RedisLayer) BatchSetKeys(ctx context.Context, kvs []util.Kv) error {
	...
	_, err := r.client.Pipelined(ctx, func(pipeliner redis.Pipeliner) error {
		for _, kv := range kvs {
			result := pipeliner.Set(ctx, kv.Key, kv.Value, time.Duration(util.RandFloatingInt64(r.ttl))*time.Second)
			if result.Err() != nil {
...
func (r *RedisLayer) SetKey(ctx context.Context, kv util.Kv) error {
	return r.client.Set(ctx, kv.Key, kv.Value, time.Duration(util.RandFloatingInt64(r.ttl))*time.Second).Err()
}
